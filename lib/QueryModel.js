...
function createAdvancedConditions (key, values, method) {
  if (Array.isArray(values)) {
    for (let i = 0, len = values.length; i < len; i++) {
      queryBuilder[method](key, '=', values[i])
    }
  } else {
    queryBuilder.where(key, '=', values)
  }
}
...
function addMoreConditions (conditions, andOr) {
  ...
  for (let key in conditions) {
    if (conditions.hasOwnProperty(key)) {
      createAdvancedConditions(key, conditions[key], method)
...
module.exports.findByFields = (conditions, columns = '*', andOr = 'AND') => {
  ...
  let firstKey = Object.keys(conditions)[0]
  queryBuilder.from(_table).select(columns)
  createAdvancedConditions(firstKey, conditions[firstKey], 'orWhere')
